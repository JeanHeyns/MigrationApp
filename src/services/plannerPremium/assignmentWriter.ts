  ...
  for (const [poProjectId, projectAssignments] of assignmentsByProject) {
    ...
    for (const chunk of chunks(projectAssignments, 180)) {
      ...
      const ops = creatable.map(assignment => {
        ...
        // niet toegestaan"). The work contour must be set afterwards via the
        // dedicated msdyn_PssUpdateResourceAssignmentContourV1 action. Until that
        // is implemented, the bare assignment is created and correctTaskSchedule
        // re-pins each task's start + duration after the resource lands.
        return {
          id: sourceId,
          dvId: assignmentId,
          entity: {
